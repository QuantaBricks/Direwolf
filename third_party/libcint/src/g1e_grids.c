...
/* rys_roots_grids_batch.c - new, separate file, only used here (COSX's
 * grids integral), not touching the shared CINTrys_roots used by g2e.c/
 * g1e.c/cint3c1e.c. See that file's own top comment. */
...
#pragma GCC ivdep
        ...
        if (omega == 0. && zeta == 0.) {
                fac1 = envs->fac[0] / aij;
                if (nroots <= 3) {
                        /* Bucketed/vectorized nroots=1..3 path - see
                         * rys_roots_grids_batch.c's own top comment. Only
                         * this (omega==0,zeta==0) branch, the common case
                         * for plain HF/hybrid-DFT exchange with no range
                         * separation - COSX's own use case. */
                        double xbatch[GRID_BLKSIZE];
                        double rbatch[GRID_BLKSIZE*3], wbatch2[GRID_BLKSIZE*3];
                        for (ig = 0; ig < bgrids; ig++) {
                                xbatch[ig] = aij * RGSQUARE(rijrg, ig);
                        }
                        switch (nroots) {
                        case 1: CINTrys_roots1_grids_batch(bgrids, xbatch, rbatch, wbatch2); break;
                        case 2: CINTrys_roots2_grids_batch(bgrids, xbatch, rbatch, wbatch2); break;
                        default: CINTrys_roots3_grids_batch(bgrids, xbatch, rbatch, wbatch2); break;
                        }
                        for (ig = 0; ig < bgrids; ig++) {
                                for (i = 0; i < nroots; i++) {
                                        u[ig+GRID_BLKSIZE*i] = rbatch[nroots*ig+i] / (rbatch[nroots*ig+i] + 1);
                                        w[ig+GRID_BLKSIZE*i] = wbatch2[nroots*ig+i] * fac1;
                                }
                        }
                } else {
                for (ig = 0; ig < bgrids; ig++) {
                ...
                if (nroots <= 3) {
                        /* Same bucketed/vectorized path as the (omega==0,
                         * zeta==0) branch above - reused verbatim here.
                         * This branch calls the very same CINTrys_roots,
                         * only with x pre-scaled by theta and the results
                         * post-scaled by theta/sqrt(theta); both scalings
                         * are per-shell-pair constants, independent of the
                         * grid point, so the batch kernel applies
                         * unchanged. Covers COSX's K_LR (erf(omega*r)/r
                         * exchange for range-separated hybrids), which
                         * previously fell through to the scalar
                         * point-by-point loop below and paid ~1.8x the
                         * full-range K cost for the same call count.
                         * Bit-identical to that scalar loop (see
                         * rys_roots_grids_batch.c's own top comment). */
                        double xbatch[GRID_BLKSIZE];
                        double rbatch[GRID_BLKSIZE*3], wbatch2[GRID_BLKSIZE*3];
                        for (ig = 0; ig < bgrids; ig++) {
                                xbatch[ig] = a0 * RGSQUARE(rijrg, ig);
                        }
                        switch (nroots) {
                        case 1: CINTrys_roots1_grids_batch(bgrids, xbatch, rbatch, wbatch2); break;
                        case 2: CINTrys_roots2_grids_batch(bgrids, xbatch, rbatch, wbatch2); break;
                        ...
                        for (ig = 0; ig < bgrids; ig++) {
                                for (i = 0; i < nroots; i++) {
                                        ...
                                        u[ig+GRID_BLKSIZE*i] = rbatch[nroots*ig+i] / (rbatch[nroots*ig+i] + 1) * theta;
                                        w[ig+GRID_BLKSIZE*i] = wbatch2[nroots*ig+i] * fac1;
                                }
                        }
                } else {
