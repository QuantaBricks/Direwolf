...
use_cache = (RSomega .eq. 0.0d0) .and. cosx_G_hi_valid
...
do ib = 0,nblocks-1
   ...
   if (use_cache) then
      call dgemm('N','N',nb,nConts,nConts,1.0d0,cosx_G_hi_a(bstart:bstart+nb-1,:),nb,Da,nConts,0.0d0,Ha,nb)
      call dgemm('N','N',nb,nConts,nConts,1.0d0,cosx_G_hi_b(bstart:bstart+nb-1,:),nb,Db,nConts,0.0d0,Hb,nb)
   else
