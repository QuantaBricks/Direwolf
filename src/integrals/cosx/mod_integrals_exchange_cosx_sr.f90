...
! COSX short-range K_SR (erfc(omega*r)/r) builder - split out of mod_integrals_exchange_cosx.f90

submodule (mod_integrals:exchange_cosx_impl) exchange_cosx_sr_impl
contains
...
if (.not. cosx_ready .or. cosx_nBases_cached .ne. nBases) then
   print *, "COSX-SR: called before the full-range COSX grid/screening ", &
            "cache was built - scf_fock.f90 must call exchange_build_cosx ", &
            "before exchange_build_cosx_sr in the same Fock build"
   call flush(6)
   stop 1
endif

if ((.not. cosx_esp_bound_sr_ready) .or. (cosx_esp_bound_sr_omega .ne. RS_omega)) then
   if (allocated(cosx_esp_bound_sr)) deallocate(cosx_esp_bound_sr)
   allocate(cosx_esp_bound_sr(0:nBases-1,0:nBases-1))
   cosx_esp_bound_sr = 0.0d0
      ...
      do sj = si,nBases-1
         ...
         do pi_b = 1,ni_b
            ...
            do pj_b = 1,nj_b
               ...
               p_b = ei_b + ej_b
               cosx_esp_bound_sr(si,sj) = cosx_esp_bound_sr(si,sj) + &
                  abs(ci_b*cj_b)*exp(-r2_b*ei_b*ej_b/p_b) * &
                  2.0d0*acos(-1.0d0)/p_b * &
                  (1.0d0 - RS_omega/sqrt(p_b+RS_omega*RS_omega))
            enddo
         enddo
         cosx_esp_bound_sr(sj,si) = cosx_esp_bound_sr(si,sj)
      enddo
   enddo
   !$omp end parallel do
   cosx_esp_bound_sr_max = maxval(cosx_esp_bound_sr)
   call cosx_build_ext_sr(nBases)
   cosx_kscreen_sr = cosx_kscreen
   cosx_kscreen_incr_sr = cosx_kscreen_incr * (cosx_esp_bound_sr_max/cosx_esp_bound_max)
   cosx_esp_bound_sr_ready = .true.
   cosx_esp_bound_sr_omega = RS_omega
endif
...
if (do_incremental) then
   ...
   do si = 0,nBases-1
      D_weighted_max(si) = maxval(cosx_esp_bound_sr(si,:) * D_pair_max(si,:))
   enddo
   !$omp end parallel do
else
   global_D_max = 0.0d0
...
do ib = 0,nblocks-1
   ...
   if (do_incremental) then
      ...
      do kk = cosx_blkptr_p(ib)+1, cosx_blkptr_p(ib+1)
         si = cosx_blkidx_p(kk)
         do sj_idx = cosx_ext_ptr_sr_sym(si)+1, cosx_ext_ptr_sr_sym(si+1)
            sj = cosx_ext_idx_sr_sym(sj_idx)
            ...
            if (r_far .gt. 0.0d0) then
               esp_eff = min(cosx_esp_bound_sr(si,sj), cosx_esp_mono(si,sj)/r_far)
            else
               esp_eff = cosx_esp_bound_sr(si,sj)
            endif
      ...
      if (block_bound .lt. kscreen_use_sr) then
   ...
   do si = 0,nBases-1
      ...
      do sj_idx = cosx_ext_ptr_sr(si)+1, cosx_ext_ptr_sr(si+1)
         sj = cosx_ext_idx_sr(sj_idx)
         ...
         if (do_incremental) then
            ...
            if (pair_bound .lt. kscreen_use_sr) cycle
         endif
         k_bound = x_block_max*esp_eff*max(F_gmax(si),F_gmax(sj))
         if (k_bound .lt. kscreen_use_sr) cycle
...
if (engine_verbose .ge. 2) then
   print '(A,I1,A,ES9.2,A,I0,A,I0,A,I0,A,I0,A)', "COSX-SR: stage=",stage_now, &
         " global_D_max=",global_D_max," skipped ",skip_total,"/",nblocks," blocks (", &
         skip_pts_total,"/",npts," grid points)"
   print '(A,I0,A,I0,A,I0,A)', "COSX-SR:   geo-pairs entered=",n_geo_total, &
...
if (do_incremental) then
   Ka = Ka + cosx_Ka_prev_sr
   Kb = Kb + cosx_Kb_prev_sr
endif

if (allocated(cosx_Ka_prev_sr)) deallocate(cosx_Ka_prev_sr,cosx_Kb_prev_sr)
allocate(cosx_Ka_prev_sr(nConts,nConts), cosx_Kb_prev_sr(nConts,nConts))
cosx_Ka_prev_sr = Ka
cosx_Kb_prev_sr = Kb
if (allocated(cosx_D_incr_a_sr)) deallocate(cosx_D_incr_a_sr,cosx_D_incr_b_sr)
allocate(cosx_D_incr_a_sr(nConts,nConts), cosx_D_incr_b_sr(nConts,nConts))
cosx_D_incr_a_sr = Da
cosx_D_incr_b_sr = Db
cosx_incr_valid_sr = .true.
if (stage_changed) then
   cosx_stage_settle_sr = 1
else if (cosx_stage_settle_sr .gt. 0) then
   cosx_stage_settle_sr = cosx_stage_settle_sr - 1
endif
cosx_stage_prev_sr = stage_now
...
end submodule exchange_cosx_sr_impl
