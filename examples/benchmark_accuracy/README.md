...
Builds nothing itself (build Engine first via `make` from the repo
root) - runs every `.inp` here against the `Engine` binary, diffs
...
| water_camb3lyp.inp | CAM-B3LYP (DF/RI) | -76.3556819885017006 (Psi4 `set puream true`) | -76.355681491 | ~5e-7 | reference updated 2026-08-03: old -76.355606583 was Psi4 default Cartesian-aux; Engine defaults to spherical since commit 2493143 and matches Psi4 puream=true (the reference update's own aux representation) to ~5e-7. force also verified, see commits 3a3fa47/7a44bcf |
...
| water_m062x.inp | M06-2X | -76.3489752858216804 (Psi4) | -76.348994378 | **~1.9e-5** | KNOWN, investigated, unresolved residual - see mod_xc.f90's M06-2X case comment. Not RI/grid/coefficient-extraction/thread-order related (all individually ruled out); PySCF (-76.34897576455575) agrees with Psi4 to ~1e-6, not with Engine. Force also verified vs Psi4 to ~1e-6 Ha/Bohr despite the energy residual (commit 2a14945-era work) |
...
| water_pbe0_d3.inp | PBE0-D3 (Grimme zero-damping, "GD3") | -76.30111300939376 (Psi4) | -76.301112520 | ~4.9e-7 (same order as plain PBE0's own baseline) | isolating the dispersion term alone (Engine/Psi4 each minus their own plain-PBE0 energy) matches to ~1e-9 Ha - force (analytic gradient) verified the same way on both this molecule and octane (26 atoms, real sp3 CN variation), matching to ~1e-9 Ha/Bohr |
...
| Quantity | Psi4 (`tests/dft-vv10`, 20x50 NLC grid) | Engine (20x50 NLC grid) | Notes |
|---|---|---|---|
...
- Engine's own grid is already converged for these functionals -
...
- Two independent reference codes (Psi4, PySCF) agree with EACH OTHER
  to ~1e-6, but both disagree with Engine by ~10-20x that.

Most likely explanation: a genuine, small systematic bias between
Engine's own (self-converged) grid quadrature scheme and Psi4/PySCF's
...
Engine - "smoother" functional forms (PBE, B3LYP, CAM-B3LYP, TPSS, all
