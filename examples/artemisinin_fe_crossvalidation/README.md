...
- **Key feature**: near-degenerate frontier orbitals (alpha HOMO ≈ beta LUMO)
  cause multi-cycle DIIS oscillation before the SCF settles. Engine
  auto-widens the DIIS window to 15 for any d/f-block metal; this system
  then converges in ~37 cycles (RI-JK) / ~51 (RIJCOSX).

## Files

### `artemisinin_fe_wb97mv.inp` — Engine, WB97M-V / def2-TZVPP (992 bf)
...
### `artemisinin_fe_wb97mv_def2tzvp.inp` — Engine, WB97M-V / def2-TZVP
...
def2-TZVPP Engine run and claimed 3e-5 Ha agreement with the 6-31G ORCA
number. Both were wrong: total energies in two different basis sets
cannot agree to 3e-5, and no Engine build (checked back to 2026-08-11)
