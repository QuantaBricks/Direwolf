...
# binary-bundle build).
RUN make MARCH=x86-64-v2 -j"$(nproc)" Direwolf

# ---- stage 2: minimal runtime ----------------------------------------------
FROM debian:bookworm-slim AS runtime

# Direwolf links libgfortran/libgomp/libquadmath/libgcc_s dynamically (see
# Makefile's Direwolf: link line); everything else (libcint, OpenBLAS, libxc)
